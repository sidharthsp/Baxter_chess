...
if __name__ == '__main__':
    ...
    claw = PickAndPlaceMoveIt("left", 0.15)
    ...
    spawn_point = Pose(position = Point(x = 0.5, y = 0.7, z = 0.78))
    frame_dist = 0.025
    model_path = rospkg.RosPack().get_path('chess_baxter')+"/models/"
    # claw.move_to_start(spawn_point)
    ...
    for row, each in enumerate(board_setup):
        ...
        for col, piece in enumerate(each):
            ...
            if piece in list_pieces:
                ...
                # srv_call(piece,pieces_xml[piece],"/",spawn_point, "world")
            srv_call(list_pieces[0],pieces_xml[list_pieces[0]],"/",spawn_point,"world")
